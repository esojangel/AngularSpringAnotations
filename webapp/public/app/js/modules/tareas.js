(function(angular){
  ...
  */
  angular.module('app.tareas', [])
    .controller('TodoCtrl', ['$scope','TareasSrv', function ($scope,TareasSrv) {
    	$scope.items = [
    	   {titulo:"mi primera tarea",des_tarea:"descripcion de tarea",complete:false}
    	];
    	TareasSrv.listar().success(function(data,code){
    		$scope.items = data;
    		console.info(data);
    	});
    }])
    .service('TareasSrv', ['$http','$q',function ($http,$q) {
      this.listar = function(){
    	  return $http.get('./tareas/listar');
      };
    }]);
})(angular);

angular.module('app.tareas').controller("TodoController", ['$scope', '$filter', function($scope, $filter) {
